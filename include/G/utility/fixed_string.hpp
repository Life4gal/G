...
namespace gal::gui::utility
{
	namespace fixed_string_detail
	{
		...
		struct basic_fixed_string : detail::constexpr_string_base<basic_fixed_string<T, N>, T>
		{
			using value_type					  = T;
		...
		template<typename T>
		struct basic_fixed_string_view : detail::constexpr_string_base<basic_fixed_string_view<T>, T>
		{
			using value_type	 = T;
			using size_type		 = std::size_t;
			using const_iterator = const value_type*;
